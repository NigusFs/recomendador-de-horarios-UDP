...
def get_preferencias(): #se deben agregar mas preguntas para tener una mejor asignacion de pesos
	...
	# se le agrego un +2 porque se esta tabajando con exponencial, si se ingresa un 1 no tendra relevancia esta ponderacion.
...
def get_peso(value_encuesta,node): 
	...
	ponderacion_parametros ={"horarios":7,"profesores":5,"ramos_dificiles":3}  # Aqui se define la importancia de los parametros
...
### CONDICIONES HORARIAS
	...
	if value_encuesta["ramos_1725"] == 3+2: 
		peso_ponderado += value_encuesta["ramos_1725"]**5 # si la respuesta de la encuesta es 3 se le asigna una importancia estandar #bien se puede sumar 0
	elif value_encuesta["ramos_1725"] > 3+2: #si al alumno le importa este valor, si se pilla la condicion se le da mas importancia al nodo si no se le agrega menos peso.
		if 	bloque_g >0:
...
def main():
	...
	print("Ramos por tomar: ", arr_ramos_criticos)
